...
def legal(message):
    """Handles the /legal command, displaying privacy policy and legal notice."""
    legal_text = "📜 Privacy Policy & Legal Notice\n\n" \
                 "This bot only scrapes public data (headlines, images, descriptions, videos).\n" \
                 "No login data, private info, or illegal activity allowed.\n\n" \
                 "Data collected is not stored permanently on our servers after being sent to you.\n" \
...
def handle_scrape(message):
    ...
    if user_data[uid]['uses'] <= 0:
        payment_info = (
            ...
            f"- **Telegram Username:** @{config.TELEGRAM_USERNAME} (or User ID: `{config.ADMIN_ID}`)\n\n"
...
# --- Admin Commands (Placeholder implementations) ---
...
def broadcast(message):
    ...
    for uid_str in user_data.keys(): # Iterate through all known user IDs
        try:
...
def stats(message):
    ...
    total_uses_granted = sum(data['uses'] for data in user_data.values())
    banned_users = sum(1 for data in user_data.values() if data.get('banned', False)) # Use .get for safety
    
    stats_text = (
        "📊 Bot Usage Statistics:\n"
        f"👥 Total registered users: `{total_users}`\n"
        f"💳 Total uses (remaining) across all users: `{total_uses_granted}`\n"
...
def ban_user(message):
    ...
    try:
        ...
        if target_uid not in user_data:
            user_data[target_uid] = {'uses': 0, 'banned': True} # Initialize as banned
        else:
